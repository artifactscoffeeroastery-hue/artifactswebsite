...
- **Not yet done, worth Denzel deciding on:** whether to backfill purchase points for real historical completed orders that predate this fix (their `point_events` 'purchase' rows never got created). Would need a similar backfill query joining `orders` (status='complete') against `customers`/`point_events` to find gaps, then crediting the missing points — not written yet, ask if wanted.
- **Copy/data mismatch spotted, not resolved:** the signup panel says "Create your account and earn 150 points instantly" and the birthday field says "for 150 bonus points", but the actual `point_action_config` value for `event_type='signup'` is 300 (confirmed — test account got credited 300, not 150). Either the copy or the config is wrong; ask Denzel which is intended.
