...
- `MAIL_FROM` — customer receipt sender. Domain `artifactscoffee.co.za` is now verified in Resend; set/confirm this is `Artifacts Coffee <hello@artifactscoffee.co.za>` in Netlify (was still the `onboarding@resend.dev` sandbox default as of 2026-08-10 — same Resend account/domain now also used for Brew Circle Auth SMTP, see Session 18).
